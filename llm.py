...
def _generate_response(prompt: str, llm_provider: str = None) -> str:
    ...
    新增支持：
    - llm_provider = "302ai" 或 "open302"
      使用 config.app 中的 open302_xxx 配置，通过 OpenAI 协议访问 https://api.302.ai/v1
    """
    content = ""
    if not llm_provider:
    ...
    account_id = None  # for cloudflare
    ...
    if llm_provider == "moonshot":
        ...
        # ollama API 不校验 key，但 openai 客户端需要一个字符串
        api_key = "ollama"
    ...
    # ✅ 新增：302.ai / open302（OpenAI 兼容网关）
    elif llm_provider in ["302ai", "open302"]:
        api_key = config.app.get("open302_api_key")
        model_name = config.app.get("open302_model_name")
        base_url = config.app.get("open302_base_url", "")
        # 自动补全 /v1，兼容你只填 https://api.302.ai 的情况
        if not base_url:
    ...
    # ----------------- Provider 特殊实现（qwen / gemini / cloudflare / ernie） -----------------
    ...
    # ----------------- 通用 OpenAI / Azure OpenAI 分支（包括 302.ai） -----------------
...
def _generate_response_video(prompt: str, llm_provider_video: str, video_file: Union[str, TextIO]) -> str:
    """
    多模态能力大模型（目前仍使用 Gemini）
    如果后面要改成 302.ai / OpenAI 视觉，可以在这里扩展分支。
    """
...
**输入示例：**  
```text  
在一个昏暗的小巷中，主角缓慢走进，四周静谧无声，只有远处隐隐传来猫的叫声。突然，背后出现一个神秘的身影。  
[  
    {  
...
## 🧩 配置 302.ai 的方式（核心就 3 个字段）

在 `config.toml`（或后台 UI）里这样填就行：
...
# 302.ai 文案模型配置
open302_api_key = "你的302AI密钥"
open302_model_name = "deepseek/deepseek-chat"   # 或其它 302 支持的模型
open302_base_url = "https://api.302.ai"        # 不写 /v1 也可以，代码会自动补
